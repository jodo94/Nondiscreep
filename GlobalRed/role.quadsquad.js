 ...
 * * New Feature: Auto-Advance. If the current room is cleared, the squad moves to the next designated target room.
...
module.exports = {
    ...
    run: function(creep) {
        if(Memory.QSMode == 1){
            // --- Configuration (Made dynamic via memory to support advancing) ---
            const INITIAL_TARGET_ROOM = 'E43S55';
            const NEXT_TARGET_ROOM = 'E43S54'; // The room to advance to next

            // Initialize the persistent target room in memory
            if (!creep.memory.targetRoomName) {
                creep.memory.targetRoomName = INITIAL_TARGET_ROOM;
            }
            const currentTargetRoomName = creep.memory.targetRoomName;
            ...
            switch (creep.memory.state) {
                case 'TRAVEL':
                    ...
                    if (creep.room.name !== currentTargetRoomName) {
                        creep.say('On my way!');
                        const destination = new RoomPosition(25, 25, currentTargetRoomName);
                ...
                case 'ATTACK':
                    ...
                    // --- Auto-Advance/Rally Logic ---
                    
                    // If no targets found and we are in the initial target room, advance to the next room.
                    if (creep.room.name === INITIAL_TARGET_ROOM) {
                        creep.memory.targetRoomName = NEXT_TARGET_ROOM; // Set the new target
                        creep.memory.state = 'TRAVEL'; // Go back to travel state
                        creep.say('Advancing to ' + NEXT_TARGET_ROOM);
                        
                        // Move immediately to prevent loitering on the spot
                        const nextDestination = new RoomPosition(25, 25, NEXT_TARGET_ROOM);
                        creep.moveTo(nextDestination, { visualizePathStyle: { stroke: '#00ff80' } });
                        return;
                    } 
                    
                    // If no targets found and we are in the NEXT_TARGET_ROOM (or beyond the defined path), rally.
                    else if (creep.room.name === NEXT_TARGET_ROOM) {
                        creep.say('Rallying');
                        creep.moveTo(new RoomPosition(25, 25, creep.room.name), { visualizePathStyle: { stroke: '#ffaa00' } });
                        return;
                    }
                    
                    // If creep is in any other room but has cleared its target, revert to TRAVEL state
                    // to ensure it heads toward the correct, currently set targetRoomName.
                    else {
                        creep.memory.state = 'TRAVEL';
                        creep.say('Re-routing');
                        return;
                    }

                    // The break is technically unreachable if all branches return, but good practice for switch cases.
                    break;
            }
        } else {
            // Placeholder for when QSMode != 1
            creep.say('QS Off');
        }
    }
};
